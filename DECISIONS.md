...
9. **Printed party vs. free choice (hf7y/american-cycle#15)** — RULED 2026-09-02: a two-arm experiment, graded on historical fidelity. Built opt-in behind `game.partyChoice` (`'printedAffinity'` / `'free'`, unset keeps the printed-party shipped behaviour exactly). `findings/party-choice.ts` measures both arms against C1b/C4's shopping ratio on `tuned.json` AND `realigning.json`, per the ruling: both arms roughly triple the printed baseline's party-flip share and clear C4's own ratio>=2 bar in every cell, including `tuned.json`, which the ruling's own text predicted would read null — it does not, because the shopping-ratio mechanism only needs a district's standing lean sign, not accumulated drift. `'free'` shops more than `'printedAffinity'` in both configs, the ordering the historical record (five vehicles, no friction) would predict, but C1b names no numeric ratio target to grade the two arms against beyond that shared threshold, so which arm is CLOSER to history is not resolved by this pass — a judgment call, not a default. D5's realignment lag is unmoved by either arm (flat at 0 years, same as the printed baseline). Not yet adopted on any shipped config.
