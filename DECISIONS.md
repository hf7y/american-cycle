...
| Primary loss to hand, general loss to discard | Primaries cheap to enter; the intended cost was card reveal, but reveal is not modelled — nothing records or prices having been seen (hf7y/american-cycle#160). **Attrition is not modelled either way** — the discard reshuffles into the talon the moment it empties, so a general loss is a lap of the track, not a drain (hf7y/american-cycle#144) |
...
2. ~~Base hand size and office bonuses~~ — **half-answered, hf7y/american-cycle#158, correction 2026-09-20, bounded 2026-09-20**: the knob survives under the same name and field (`hand.base`/`bonusX`), but hf7y/american-cycle#158 split what it sizes. Districts stopped competing with candidates for a shared pack-pass slot the moment they stopped being drafted at all (`draft.districtsDealt`, dealt once at construction) — `hand.base` now sizes the CANDIDATE draft alone (`Game.draftCandidates`), not a combined hand of both kinds. Concretely: it no longer sets board size (open-race count) the way hf7y/american-cycle#77/#90 measured it doing jointly with districts before. `Game.dealMoreDistricts`, driven by `draft.districtsPerCycle` (1 on every shipped config), hands each player one more district — but was firing on EVERY election cycle for the rest of the game regardless of anything else, on top of the `districtsDealt` starting deal, with nothing bounding it: linear and unbroken for the full `game.maxYears`, since a district admitted never returns to the discard the way a defeated candidate does. Measured consequence at the time: the House chamber (currently-held representative seats) grew from a stable ~30-40 under the old mechanic to 80+ by year 20, and the CI board check's fixed seed 2 over a full 100-year game saw zero Fed reactions at all (`rateRises` needs a passed bill to ever load the track, and none ever passed). **Bound, same day**: `refill()` now fires the trickle only on a cycle whose candidate draft actually exhausted the talon and pulled in a new era pack (`Game.eraQueue` shrinking), the same event that exposed a later era's districts under the old shared pack-pass draft this replaced — capping the lifetime trickle at one dose per era the pack ever had (six more after the starting deal) instead of one per election year for up to `game.maxYears`. Re-measured, `turn-loop-redesign-158`, `as-written-plus.json`, `Greedy/Greedy/HouseFarm/Random`, 10 seeds, `maxYears: 20`: House averages ~58 held seats (was 80+, still above the old mechanic's ~30-40) and 10 bills pass across the 10 games (was 2 across 20 games); the CI board check's seed 2 now passes 3 bills and sees 2 Fed reactions over the full 100 years. Bill passage and chamber size are still both higher than the old mechanic's, so this is a bound on the runaway, not a re-tuning to parity — whether `districtsPerCycle`, `districtsDealt`, or the vote's own `VOTE_AT_DISTANCE` need recalibration against this new pacing is unmeasured and remains open.
3. ~~District synergy magnitudes~~ — **deleted**, hf7y/american-cycle#27: a card's own personal vote is `CandidateCard.homeStateBonus`, not a district-level bonus for whoever happens to hold the card — Manchin, Collins and Edwards are properties of the candidate, not the place. `synergy` is gone from `DistrictCard` and every printed card; district fit is identity match alone, keyed on whoever is in the race (hf7y/american-cycle#40). That identity vocabulary was itself cut down 2026-09-06 (Zach, hf7y/american-cycle#162, #164): `ivy`, `veteran`, `business`, `academic` are gone -- biography typed as if it were geography, oversupplied on candidates in exactly the ratio the place tags were oversupplied on districts -- leaving eleven tags, first step of a five-part sourcing programme (#164) the rest of which is not done.
