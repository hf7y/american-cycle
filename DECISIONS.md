...
3. ~~District synergy magnitudes~~ — **deleted**, hf7y/american-cycle#27: a card's own personal vote is `CandidateCard.homeStateBonus`, not a district-level bonus for whoever happens to hold the card — Manchin, Collins and Edwards are properties of the candidate, not the place. `synergy` is gone from `DistrictCard` and every printed card; district fit is identity match alone, keyed on whoever is in the race (hf7y/american-cycle#40). That match runs on a vocabulary since found unsourced and partly biography rather than geography: `ivy`/`veteran`/`business`/`academic` are now cut from `IdentityTag` entirely, not just off districts (hf7y/american-cycle#162, #164) — sourcing what survives from real data, census first, stays open there.
