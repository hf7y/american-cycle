...
2. ~~Base hand size and office bonuses~~ — **half-answered, hf7y/american-cycle#158, correction 2026-09-20**: the knob survives under the same name and field (`hand.base`/`bonusX`), but hf7y/american-cycle#158 split what it sizes. Districts stopped competing with candidates for a shared pack-pass slot the moment they stopped being drafted at all (`draft.districtsDealt`, dealt once at construction) — `hand.base` now sizes the CANDIDATE draft alone (`Game.draftCandidates`), not a combined hand of both kinds. Concretely: it no longer sets board size (open-race count) the way hf7y/american-cycle#77/#90 measured it doing jointly with districts before. **Correction: the previous entry here claimed district count is "fixed at deal time" — it is not.** `Game.dealMoreDistricts`, driven by `draft.districtsPerCycle` (1 on every shipped config), hands each player one more district EVERY election cycle for the rest of the game, on top of the `districtsDealt` starting deal — unlike the old pack-pass draft, nothing bounds this: it is linear and unbroken for the full `game.maxYears`. That is exactly the "board size" role hf7y/american-cycle#77/#90 found `hand.base` setting jointly before, and nothing has replaced it — acceptance item 3's own escape clause ("or report that nothing does") is the honest answer for districts specifically. Measured consequence, `turn-loop-redesign-158` vs `main`, `as-written-plus.json`, `Greedy/Greedy/HouseFarm/Random`, matched seeds, `maxYears: 20`: the House chamber (currently-held representative seats) grows from a stable ~30-40 under the old mechanic to 80+ by year 20 as the trickle keeps admitting new, tag-diverse districts; the tag-distance vote (`sim/agents.ts`'s `Base.voteBill`) then finds a shrinking share of an ever-widening chamber within `VOTE_AT_DISTANCE` of any one bill, and bill passage collapses (2/20 attempts passed on the branch vs 20/48 on `main` at year 20, same 10 seeds; the CI board check's fixed seed 2 over a full 100-year game sees zero Fed reactions at all — `rateRises` needs a passed bill to ever load the track). Whether `districtsPerCycle` needs a cap, a decay, or a different lever entirely on game length/runaway is unanswered and is the actual open half of item 3.
