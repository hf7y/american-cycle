...
| **CUT `heterodox`. It is a net liability to the card that carries it.** | MEASURED over 40 games, 28,227 general sides. The exemption WORKS — 0.00% of 11,611 heterodox sides carry a national modifier against 43.95% of orthodox sides — and it is worth **+0.316 pips** averaged over a heterodox card's generals, **7.6% of the general's 4.18-pip SD**, below the game's own 1-pip granularity. Meanwhile `heterodoxPrimaryPenalty` charges **−2**, and a primary is 1d6 (SD 2.42) so that −2 is 0.83 SD. **114 of 346 cards pay 2 pips in the round that is 100% contested to save 0.32 in the round that is 83% walkovers.** The reason it is small: national modifiers fire only when `ctx.presidentParty` matches the card's party, so 56% of sides never see one, and the mean is −0.72 when present. The primary half is already duplicated, earned, by cross-benching. Nothing survives that needs a tag. |
| ~~derive `heterodox`~~ superseded by the row above | NOT retired -- it is live on 114 of 346 candidates and does two jobs: `heterodoxPrimaryPenalty` and the §9 exemption from national modifiers (`resolution.ts:54`). Under the mapping above a candidate is heterodox WHEN THEIR DEMOGRAPHIC FIT POINTS AWAY FROM THEIR PARTY'S CURRENT ERA MAPPING, which recovers both jobs and makes heterodoxy era-dependent -- Manchin is heterodox in 2018 and orthodox in 1958, which a printed tag cannot express. The lag needs no field: when the mapping flips, the incumbent holds on native-son plus fit plus incumbency for a few cycles. Measured real lag ~10 years; a Senate term is 6 and a House term 2. |
| **BUILT BUT INERT — four knobs shipped at a no-op default** | `statewideIdentity` ('district', measured 48.80%→43.22%), `crossBenchGeneral` (0), `launchpad.senator` and `.representative` (0/0 — the senator PRIMARY row was the whole reason for the table), `resignToRun` (false). Each was left off with "measure before defaulting" and none was returned to. That is a pattern, not four accidents: the discipline that catches errors also strands work. Decide each one or delete it. |
| **`crossBenchGeneral` keys on the wrong thing** | Measured: **74% of races where it would fire sit at `|lean| <= 3`**, because a hardened state's general is a walkover 96% of the time. It rewards defection-with-the-drift precisely where there is no drift. The knob does work (with-minus-against moves −36.4 / −11.6 / −2.9 / +10.2), but its trigger is mis-aimed. Fix the trigger or drop the term. |
| **no test covers the PRIMARY distribution** | `resolution.test.ts` guards §3's odds table, which BUILD-BRIEF calls "the foundation the rest of the game sits on" — and it is the general's table only. Zero occurrences of `1d6` or `primary` in that file. The round that is 100% contested is untested. |
| **`RaceEvent.sides` drops `heterodox`** | `Side` carries it, the event's inline side type does not, so it is unobservable from a replay. This produced a false zero here and cost two wrong probes. |
| **odd-year governors never pursued** | Zach's original question — running in odd years to bank incumbency before an even-year Senate run — was measured as 1,039 even-year governor races against 0 odd, and then dropped. `oddYearGovernors` is not even present in the configs. |
| **`bill-passage-is-the-table.ts` contradicts `FINDINGS.md`** | The predicate stamps passage at **12%** and the 50/67 thresholds at 41%/2%; F14/F27 and both published reports say **16%**/63%/0%. Flagged during the report reconciliation with "someone should decide which is canonical." Nobody has. |
| **`report.html` carries an unverifiable figure** | "Localism declines from pack rotation alone — 4.45 → 1.50" matches no finding; the markdown's own four-pack figure is 2.96 → 1.62. Left in place rather than invented over. |
| **CI is green while findings are stale** | It runs `sim/findings.ts` at `FINDINGS_SEEDS: 12`, where two findings read STALE, and only BROKEN fails the step. The gate cannot catch staleness by construction. That is defensible — small N proves predicates RUN, not that headlines HOLD — but it means CI green is not evidence a headline is current. |
| **LAYERED, NOT REPLACED: the stepping stone has two spellings** | `d.incumbent = holdsThis \|\| isGovernor` still gives governor→Senate a BORROWED incumbency pip, while governor→president goes through the `launchpad` table. I replaced half the mechanism and left the other half. One concept, two implementations. |
| **LAYERED, NOT REPLACED: defection is priced twice in the primary** | `heterodoxPrimaryPenalty` (printed, −2) and `crossBenchPrimaryPenalty` (earned, −1 x count, capped 3) both fire in the same branch, `elections.ts:180` and `:183`. Cutting `heterodox` resolves this; until then a card can pay both. |
| **three data files nothing reads** | `data/historical/governor-rules.json`, `house_district_panel.json`, `pres_state_panel.json` are committed and referenced only by scratch analysis scripts. Fine as evidence, but nothing in `engine/` or `findings/` consumes them. |
