...
const OPPONENT_BLURB = {
  ...
  Random:'Gentlest. Declares anywhere; a floor rather than a rival.',
  HouseFarm:'Gentle. Builds district presence and promotes upward, but spreads too thin to convert it.',
  SenateFlood:'Strong. Runs everything for Senate early, for the six-year terms and the hand size.',
...
const SOLITAIRE_BLURB = {
  ...
  HeterodoxSpecialist:'Fair 1v1. You should win about 59% of these.',
  BillMaximizer:'Very gentle 1v1. You should win about 77% of these.',
  Impeacher:'Hard 1v1. Wins about 66% of the time.',
  EconomyChicken:'Fair 1v1. You should win about 54% of these.',
  WideAndEmpty:'Unloseable 1v1. Should be a losing strategy, and is: it has not won one game in this measurement.',
...
// #46: which of the seventeen give a fair 1v1, not just the field's fair
// share. Tiers read off SOLITAIRE_BLURB's own measured win rates
// (findings/board-opponent-fitness.ts, #205/#209) -- fair is the 40-60%
// human-win band, the same band that finding calls out by name. Greedy has
// no separate 1v1 measurement (it IS the human proxy the others are duelled
// against), so it's grouped fair by that construction.
...
function setup() {
  ...
  // s1 doubles as the true-1v1 seat when s2/s3 stay blank, so group it by
  // SOLITAIRE_TIER instead of the flat list s2/s3 use -- the picker itself
  // now shows which of the seventeen are a fair fight alone.
  ...
  modal(`
    ...
    <div class="row" style="margin-top:14px">
      ...
      <p class="note" style="max-width:46ch"><b>If this is your first game:</b> a seat is worth what it
      pays over its whole term, not what it scores today. Measured over 200 games, winners end holding
  ...
  // Measured default: a player of ordinary skill wins 27% here (fair share is
  // 25%) and a strong one 70%. Against Lookahead + HouseFarm + Greedy the same
  ...
  const sync = () => {
    ...
    $('b1').textContent = !$('s1').value
      ...
      : soloOpponent ? (SOLITAIRE_BLURB[$('s1').value] || '') : (OPPONENT_BLURB[$('s1').value] || '');
