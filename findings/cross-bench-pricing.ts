...
export const finding: Finding = {
  ...
  stampedAt: '2026-09-04T06:14:34Z',
  stampedOn: 'd0bd7ed',
  ...
  predicate(): Claim[] {
    ...
    return [
      ...
      { name: 'score gap by EXCESS concentration (median split)', value: m.excessGap, stamped: -3.33, tolerance: 40 },
      { name: 'score gap by RAW Herfindahl (the confound)', value: m.rawSplit.gap, stamped: -13.31, tolerance: 40 },
      { name: 'seats riding along with the raw split', value: m.rawSplit.seatGap, stamped: -4.49, tolerance: 6 },
      ...
      { name: 'lone cross-bencher wins the primary, unpriced', value: 100 * m.unpricedPrimary, stamped: 61.74, tolerance: 7, unit: '%' },
      { name: 'lone cross-bencher wins the primary, at the shipped penalty', value: 100 * m.shippedPrimary, stamped: 47.05, tolerance: 7, unit: '%' },
      { name: 'mean pips the primary penalty applies, in PRIMARY SDs', value: m.shippedPips / PRIMARY_SD, stamped: 1.09, tolerance: 0.5 },

      // --- why a signed general term was measured and then cut ---
      { name: 'races a signed general term would fire in that are |lean| <= 3', value: 100 * m.purpleShare, stamped: 62.86, tolerance: 12, unit: '%' },

      // --- the walkover asymmetry that excuses the primary from a lean condition ---
      { name: 'primary:general contested ratio, purple states (|lean| 0-1)', value: m.purple, stamped: 1.06, tolerance: 0.25 },
      { name: 'primary:general contested ratio, hardened states (|lean| 6-8)', value: m.hardened, stamped: 36.36, tolerance: 3.5 },
