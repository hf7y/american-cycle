...
export const finding: Finding = {
  ...
  headline:
    ...
    + 'same 120-seed baseline runaway-no-brake.ts uses (restamped 2026-09-21 against `main`; the engine has moved '
    + 'since this was first drafted 2026-09-17, so the shares below are noticeably more lopsided than the first '
    + 'pass found, though the conclusion is unchanged): presidential rows (row%4==0) carry only 28.0% of all '
    + 'lead changes and 22.6% of the LAST lead change per game -- close to the 25% a uniform spread across '
    + 'four residues would give, i.e. presidential years are not even mildly special. The dominant residue is '
    + 'row%4==2 -- MIDTERM years -- at 47.9% of all changes and 58.5% of last changes. That has a named cause, '
    ...
    + 'only, no federal race and no national modifier) split the remaining ~19% unevenly. Per the ruling\'s own '
    + 'branch: hypothesis rejected, so no band redefinition and no restamp of the #84 arms follows from this '
    + 'finding alone -- the 10/16-vs-14/16 clustering those three sweeps hit is better explained by which '
    + 'specific configs happened to be swept than by a strict 4-year gate on the metric. Separately, and worth '
    + 'flagging on #84: the shipped baseline (incumbency ON, no ablation) now measures determination 0.875 at '
    + 'full 120-seed precision, not the 0.63 runaway-no-brake.ts has stamped since 2026-09-05 -- the engine has '
    + 'moved enough on its own (identityWeights rollout #19, roll-call votes #263, and other churn since) that '
    + "runaway-no-brake.ts is due a fresh look independent of this finding's own question.",
  stampedAt: '2026-09-21T00:00:00Z',
  stampedOn: '6397b56',
  ...
  predicate(): Claim[] {
    ...
    return [
      { name: 'presidential-row share of all lead changes', value: cadence.presidentialShareOfAllChanges, stamped: 0.280, tolerance: 0.05, unit: 'share' },
      { name: 'presidential-row share of last lead change', value: cadence.presidentialShareOfLastChange, stamped: 0.226, tolerance: 0.06, unit: 'share' },
      { name: 'midterm-row share of all lead changes', value: cadence.totalChanges ? cadence.residueChanges[2] / cadence.totalChanges : 0, stamped: 0.479, tolerance: 0.06, unit: 'share' },
      { name: 'midterm-row share of last lead change', value: cadence.gamesWithChange ? cadence.residueLastChange[2] / cadence.gamesWithChange : 0, stamped: 0.585, tolerance: 0.07, unit: 'share' },
