 ...
 *  The thread's split by tag kind found the four PERSON tags (ivy, academic,
 *  business, veteran) predicting far more weakly than PLACE/DEMOGRAPHIC ones
 *  -- consistent with "biography used as geography", not a real demographic
 *  signal. hf7y/american-cycle#164 has since RULED on exactly that and cut
 *  all four from the vocabulary (#201) -- there is no PERSON category left
 *  to split out. This predicate measures what remains: place vs. demographic
 *  tags, both drawn from a card pool that no longer carries the contested
 *  kind. */
...
function measure() {
  ...
  const ALL: IdentityTag[] = [...PLACE, ...DEMOGRAPHIC];
  return {
    ...
    nPlace: predict(matched, PLACE).length,
    nDemographic: predict(matched, DEMOGRAPHIC).length,
  };
}
...
export const finding: Finding = {
  ...
  question:
    ...
    + "card's `demographics` predict its real district-era partisanship, once split into place tags "
    + '(urban/rural/suburban) and demographic tags (catholic/evangelical/jewish/black/hispanic/cuban/'
    + 'union/farm) -- the only two kinds left in the vocabulary now that #164 has cut the four PERSON '
    + 'tags (ivy/academic/business/veteran) the same thread found predicting far more weakly?',

  headline:
    "It predicts: leave-one-out r^2 against the real district-era fixed effect is 0.57 pooled, 0.49 for "
    + 'place tags alone and 0.35 for demographic tags alone, matched against only the four packs the '
    + "MEDSL panel (1976-2018) can actually speak to. #163's own scratch probe joined 1932/1964/2024 "
    + 'cards too, on a coincidental era-bucket letter rather than a real place match, which is why this '
    + "N (153 of 168) is smaller than the 258/248 the issue thread quotes. The PERSON-tag split that "
    + 'thread also ran (r^2 0.218 on ivy/academic/business/veteran, weaker than either kind kept here) '
    + "is moot as a category now: #164 (#201) removed all four from the vocabulary, so there is nothing "
    + 'left to withdraw.',
  stampedAt: '2026-09-06T19:15:00Z',
  stampedOn: '68c2395',
  ...
  predicate(): Claim[] {
    ...
    return [
      ...
      { name: 'leave-one-out r^2, all remaining tags', value: m.all, stamped: 0.57, tolerance: 0.05 },
      ...
      { name: 'cards with a usable place-tag prediction', value: m.nPlace, stamped: 149, tolerance: 3 },
      { name: 'cards with a usable demographic-tag prediction', value: m.nDemographic, stamped: 119, tolerance: 5 },
    ];
  },

  verdict(c: Claim[]): string {
    const v = (n: string) => c.find((x) => x.name === n)!.value;
    const all = v('leave-one-out r^2, all remaining tags');
    ...
    return [
      ...
      `place tags alone predict at r^2 ${place.toFixed(2)}, demographic tags alone at ${demo.toFixed(2)}`,
      'a labeller who already knows a district\'s politics before tagging it is a confound LOO cannot '
        + "detect, per #163's own withdrawal -- this is evidence the remaining tags correlate with real "
        + 'partisanship, not evidence they are an independent source of it',
    ].join('; ');
  },
};
